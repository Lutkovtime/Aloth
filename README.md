...
Активная разработка. Текущий релиз — **0.1.x** (описание ниже). Идёт пересборка **v2** до уровня продукта: терминальный REPL, визард первого запуска, файловая память, три профиля установки, подписанные релизы. План и документация v2 появятся в репо по ходу работы. Подписанные сборки — после подключения SignPath Foundation.
